...
- Electron-based Windows app that watches the primary display and offers proactive on-screen hints powered by Anthropic Claude.
...
- Config via `ANTHROPIC_API_KEY` env var or `%APPDATA%\OpenMagicPointer\config.json`.
- Unit, integration, and end-to-end test suites (Vitest + Playwright).
